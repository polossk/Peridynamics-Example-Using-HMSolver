...
from hmsolver.femcore import point_criteria, segment_criteria, rectangle_criteria
...
def main(example_name, mesh_file_name, cname, total_phase=5):
    ...
    total_area -= notch_width * notch_depth
    total_area -= 0.5 * np.pi * (notch_width / 2)**2
    ...
    notch_points = [(zone_xl, zone_yl + notch_height),
                    ...
                    (zone_xl + notch_depth + 0.5 * notch_width,
                     zone_yl + notch_height + 0.5 * notch_width),
                    (zone_xl + notch_depth,
                     zone_yl + notch_height + notch_width),
                    (zone_xl, zone_yl + notch_height + notch_width)]
    mesh2d.initCracks([(pA, pB)
                       for pA, pB in zip(notch_points[:4], notch_points[1:])])
    ...
    boundary_5 = segment_criteria(zone_xl, zone_yr * 0.75, zone_xl, zone_yr)
    group_1 = rectangle_criteria(zone_xl, zone_yl, zone_xr, zone_yr * 0.1)
    group_2 = rectangle_criteria(zone_xl, zone_ymid, zone_xr * 0.1, zone_yr)
    group_3 = rectangle_criteria(zone_xl, zone_yr * 0.9, zone_xmid, zone_yr)
    ...
    app.manual_set_fem_zone(group_1)
    app.manual_set_fem_zone(group_2)
    app.manual_set_fem_zone(group_3)
...
if __name__ == '__main__':
    ...
    # phase             # 拟静态断裂加载步数 options: positive intergers ($\mathbb Z^+$)
    ...
    arg.add_argument("-r",
                     ...
                     default=1,
                     type=int,
                     help="sub number(runid) of simulation")
    arg.add_argument("-t",
                     "--mtype",
                     metavar="int",
                     default=1,
    ...
    arg.add_argument("-p",
                     ...
                     default=5,
                     type=int,
                     help="total phase of simulation")
    args = arg.parse_args()
    ...
    mesh_file_name = f"{args.name}{args.runid}{args.mtype}.mesh"
